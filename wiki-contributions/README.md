...
|---|---|---|
| [DML.md](DML.md) | Full rewrite: PERS container model, 38-byte header layout, per-version record layouts (v2 48B / v3 60B / v4 64B), per-field value semantics with confidence ratings, `type` enum table covering all 15 indices, runtime-consumer audit (no code branches on these fields), and pointer to the `studio::content::dml::is_dml`/`get_dml_data` wrapper plus the typed surface at `renderable_shape::get_materials()` | Hex dump of 259 standalone `.dml` files from `Entities.vol` plus the per-world `*Terrain.vol` / `human1DML.vol` archives; cross-referenced against `lib3space`'s pre-existing `studio::content::dts::darkstar::material_list` parser (commit `06d69a3`, 2022-11-22) and the new `dml::` wrapper (commit `0e4724f`). Deeper writeups in `docs/done/03-dml/`. |
...
| [DTS-ANIMATION.md](DTS-ANIMATION.md) | New page: node hierarchy + sequence + sub-sequence + keyframe + transform layout for DTS v2/v3/v5/v6/v7/v8, per-version transform shape differences (v8 has no scale; v7/v8 use `quaternion4s`), `time_in_sec = position * duration` mapping verified on larmor `run`, LOD branch convention (suffix = `detail.size`), worked hex example from `chainturret.DTS`, and sequence catalogues for player armour (larmor), vehicle (hover_apc), and turret (chainturret) | Cross-reference of `lib3space`'s pre-existing `studio::content::dts::darkstar::read_shape()` parser, byte-level inspection of `chainturret.DTS` / `larmor.dts` / `hover_apc.DTS`, and the working CPU-skinned playback path landed in `examples/dts-viewer/main.cpp` across commits `ec7a525` → `80c0515`. Deeper writeups in `docs/done/02-animation/`. |
